...
relax_phase_start = 2000;
relax_phase_end = 166654;


fs = 250; % Sampling frequency

% Segmenting the data into different phases
ecg_relax = ecg(relax_phase_start:relax_phase_end);



% Start with Realx
[qrs_amp_raw,qrs_i_raw,ecg_d, delay]=pan_tompkin(ecg_relax,fs,0);
...
QRS_duration_R = (S_off_set_Locs - Q_on_set_locs) / (k*fs) * 1000; % in ms


average_QRS = mean(QRS_duration_R); % Average of all beats
std_QRS = std(QRS_duration_R); % STD of all beats
