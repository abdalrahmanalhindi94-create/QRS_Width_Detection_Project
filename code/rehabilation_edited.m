...
rehab_phase_start = 697006;
rehab_phase_end = 807750; 

fs = 250; % Sampling frequency

% Segmenting the data into different phases
ecg_rehab = ecg(rehab_phase_start:rehab_phase_end);



% moving to Rehabilation
[qrs_amp_raw,qrs_i_raw,ecg_d, delay]=pan_tompkin(ecg_rehab,fs,0);
...
QRS_duration_Rh = (S_off_set_Locs - Q_on_set_locs) / (k*fs) * 1000; % in ms

average_QRS = mean(QRS_duration_Rh); % Average of all beats
std_QRS = std(QRS_duration_Rh); % STD of all beats
